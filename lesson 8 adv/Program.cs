...
namespace lesson_8_adv
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var bmwCars = new List<BmwCar> { new BmwCar(true, "model2"), new BmwCar(true, "model3"), new BmwCar(true, "model1") };
            var carComparer = new BmwCarComparer();
            bmwCars.Sort(carComparer);
            foreach (var c in bmwCars)
            {
            ...
            var audiCars = new List<AudiCar> { new AudiCar(false, "model20"), new AudiCar(false, "model30"), new AudiCar(false, "model10") };
            var carComparer1 = new AudiCarComparer();
            audiCars.Sort(carComparer1);
            foreach (var c in audiCars)
            {
                Console.WriteLine(c.Model);
            }
